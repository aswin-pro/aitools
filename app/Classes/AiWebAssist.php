...
class AiWebAssist
{
    ...
    public function generate($request)
    {
        ...
        if ($isSystem) {
            ...
            $defaultMessage = "Your name is WebAssist and your job is to read the content on the website and understand it and answer the questions I ask about the website content, SEO details, new page creation content, blog content, content creation, optimization, and content analysis. Don't say anything apart from the website content.";
            ...
            if (isset($request["content"])) {
                $messages[] = ['role' => 'user', 'content' => "Website: " . $request["content"]];
            }
        ...
        if (!in_array($model, self::CHAT_MODELS)) {
            Log::error("AiWebAssist: model '{$model}' is not in the supported chat-models list.");
            return null;
        }
        ...
        try {
            ...
            Log::error("AiWebAssist: OpenAI call failed: " . $e->getMessage());
            return null;
        }

        if ($response === null) {
            Log::error("AiWebAssist: received unparseable response from OpenAI.");
            return null;
        }

        if (isset($response->error)) {
            Log::error("AiWebAssist: OpenAI API error: " . json_encode($response->error));
            $this->result = null;
        } elseif (isset($response->choices[0]->message->content)) {
            $this->result = $response->choices[0]->message->content;
        } else {
            Log::error("AiWebAssist: unexpected response shape: " . json_encode($response));
            $this->result = null;
        }
