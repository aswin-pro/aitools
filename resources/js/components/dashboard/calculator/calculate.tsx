...
import { FieldType, Product, RequiredMaterialProduct } from '@/types';
...
export default function Calculate({
    ...
    const columns: FieldType<Item>[] = [
        {
            ...
            props: {
                ...
                options: assembled_products.map((assembled_product) => ({
                    label: assembled_product.product_name,
                    value: assembled_product.product_id.toString(),
                })),
                required: true,
            },
        },
        {
            ...
            inputGroup: {
                align: 'inline-end',
                content: (item: Item) => getUnit(item.product_id),
            },
        },
    ];
    ...
    const calculateRequiredMaterials = (items: Item[]) => {
        const materialMap = new Map<string, RequiredMaterialProduct>();

        items.forEach((item) => {
            const assembledProduct = assembled_products.find(
                (p) => p.product_id === item.product_id,
            );

            if (!assembledProduct?.required_materials) return;

            assembledProduct.required_materials.forEach((material) => {
                                ...
                                requiredMaterials.map((item, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell className="border-r font-medium text-red-600">
                                            {roundToThreeDecimals(item.required_qty)}{' '}
                                            {item.measuring_unit}
                                            <input
                                                type="hidden"
                                        ...
                                        <TableCell className="border-r font-medium text-blue-600">
                                            {roundToThreeDecimals(item.balance_qty)}{' '}
                                            {item.measuring_unit}
                                            <input
                                                type="hidden"
