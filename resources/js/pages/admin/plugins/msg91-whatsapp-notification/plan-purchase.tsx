...
export default function PlanPurchase({ adminTemplate, userTemplate }: Props) {
    const { t } = useTranslation();

    const [adminVariableRows, setAdminVariableRows] = useState<VariableRow[]>(
        createVariableRows(adminTemplate?.variables),
    );

    const [userVariableRows, setUserVariableRows] = useState<VariableRow[]>(
        createVariableRows(userTemplate?.variables),
    );

    const form = useForm({
        plan_purchase_admin: adminTemplate?.is_enabled === 1,
        plan_purchase_admin_template_id: adminTemplate?.template_id ?? "",
        plan_purchase_admin_template_namespace: adminTemplate?.namespace ?? "",

        plan_purchase_user: userTemplate?.is_enabled === 1,
        plan_purchase_user_template_id: userTemplate?.template_id ?? "",
        plan_purchase_user_template_namespace: userTemplate?.namespace ?? "",
    ...
    const handleAdminVariableChange = (index: number, value: string) => {
        const rows = [...adminVariableRows];
    ...
    const handleUserCheckboxChange = (index: number, checked: boolean) => {
    ...
    const handleUserVariableChange = (index: number, value: string) => {
        const rows = [...userVariableRows];
    ...
    const submit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        form.post(route("admin.msg91_whatsapp_template_plan_purchase.update"), {
            preserveScroll: true,
        ...
        <section className="space-y-6">
            ...
            <form onSubmit={submit} className="space-y-10">
                ...
                <div className="space-y-6">
                    <h3 className="text-base font-semibold">{t("Admin")}</h3>

                    <div className="grid gap-6 md:grid-cols-3">
                        {/* Enable Admin */}
                        <div className="grid gap-2">
                            <Label htmlFor="plan_purchase_admin" required>
                            ...
                            <div className="flex h-10 items-center">
                                <Switch
                                    ...
                                    checked={form.data.plan_purchase_admin}
                                    onCheckedChange={(checked) => {
                                        ...
                                        form.clearErrors(
                                            ...
                                            "plan_purchase_admin_template_namespace",
                                        );
                                    }}
                                />
                            </div>
                        ...
                        <FormInput
                            ...
                            required
                            value={form.data.plan_purchase_admin_template_id}
                            onChange={(e) => {
                                form.setData(
                                    "plan_purchase_admin_template_id",
                                    e.target.value,
                            ...
                            error={form.errors.plan_purchase_admin_template_id}
                        />

                        {/* Admin Namespace */}
                        <FormInput
                            id="plan_purchase_admin_template_namespace"
                            name="plan_purchase_admin_template_namespace"
                            label={t("Template Namespace")}
                            placeholder={t("Template Namespace")}
                            required
                            value={
                                form.data.plan_purchase_admin_template_namespace
                            }
                            onChange={(e) => {
                                form.setData(
                                    "plan_purchase_admin_template_namespace",
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="overflow-x-auto rounded-md border">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {adminVariableRows.map((row, index) => (
                                        <tr
                                            key={index}
                ...
                <div className="space-y-6">
                    <h3 className="text-base font-semibold">{t("User")}</h3>

                    <div className="grid gap-6 md:grid-cols-3">
                        <div className="grid gap-2">
                            <Label htmlFor="plan_purchase_user" required>
                            ...
                            <div className="flex h-10 items-center">
                                <Switch
                                    ...
                                    checked={form.data.plan_purchase_user}
                                    onCheckedChange={(checked) => {
                                        form.setData(
                                            "plan_purchase_user",
                                            checked,
                                        ...
                                        form.clearErrors(
                                            ...
                                            "plan_purchase_user_template_namespace",
                                        );
                                    }}
                                />
                            </div>
                        ...
                        <FormInput
                            ...
                            placeholder={t("Template Name")}
                            required
                            value={form.data.plan_purchase_user_template_id}
                            onChange={(e) => {
                                form.setData(
                                    "plan_purchase_user_template_id",
                                    e.target.value,
                            ...
                            error={form.errors.plan_purchase_user_template_id}
                        />

                        {/* User Namespace */}
                        <FormInput
                            id="plan_purchase_user_template_namespace"
                            name="plan_purchase_user_template_namespace"
                            label={t("Template Namespace")}
                            placeholder={t("Template Namespace")}
                            required
                            value={
                                form.data.plan_purchase_user_template_namespace
                            }
                            onChange={(e) => {
                                form.setData(
                                    "plan_purchase_user_template_namespace",
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="overflow-x-auto rounded-md border">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {userVariableRows.map((row, index) => (
                                        <tr
                                            key={index}
                                            className="border-b last:border-0"
                                        >
