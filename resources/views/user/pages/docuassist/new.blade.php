        ...
        <div class="container-fluid">
            ...
            <div class="page-header d-print-none">
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <h2 class="page-title">
                            {{ __('Chat with ' . $docuAssistArray['chat_genius_name']) }}
                        </h2>
                    </div>
                    {{-- New Chat Genius --}}
                    <div class="col-auto ms-auto">
                        <a href="{{ route('user.new.ai.docuassistant', $docuAssistArray['chat_genius_id']) }}"
            ...
            <div class="page-body">
                <div class="container-fluid my-auto">
                    ...
                    {{-- Mobile chats modal --}}
                    @if (count($docuAssistArray['chats']) > 0)
                                    ...
                                    <div class="modal-body">
                                        <div class="card-body p-0 scrollable">
                                            <div class="nav flex-column nav-pills" id="pills-tab" role="tablist" aria-orientation="vertical">
                                                {{-- Conversations --}}
                                                @if (count($docuAssistArray['chats']) > 0)
                                                    @foreach ($docuAssistArray['chats'] as $chat)
                                                        <a href="#chat{{ $chat['chat_id'] }}"
                                                            ...
                                                            <div class="row align-items-center flex-fill">
                                                                <div class="col-auto">
                                                                    <span class="avatar"
                                                                        style="background-image: url({{ asset($docuAssistArray['chat_genius_image']) }})"></span>
                                                                </div>
                                                                <div class="col text-body">
                                                                    <div>{{ __($chat['chat_title']) }}</div>
                                                                    <div class="text-secondary text-truncate w-100">
                                                                        {{ __($docuAssistArray['chat_genius_expert']) }}</div>
                                                                </div>
                                                            </div>
                                                        </a>
                                                                            ...
                                                                            <div class="row">
                                                                                <div class="col chat-bubble-author">
                                                                                    @if ($message['responsed_by'] == 'user')
                                                                                        {{ Auth::user()->name }}
                                                                                    @else
                                                                                        {{ $docuAssistArray['chat_genius_name'] }}
                                                                                    @endif
                                                                                </div>
                                    ...
                                    <form id="chatgenius-form">
                                        <div class="input-group input-group-flat">
                                            ...
                                            <input type="hidden" name="chat_id"
                                                value="{{ $docuAssistArray['chats'][0]['chat_id'] }}">
                                            ...
                                            <span class="input-group-text">
                                                <button type="button" class="btn btn-primary btn-icon ms-1"
                                                ...
                                                <input type="file" name="uploadFile" id="uploadFile"
                                                    class="form-control" accept="application/pdf,text/plain"
                                                    style="display: none;" onchange="validateFile()">
...
@section('custom-js')
    ...
    <script>
        ...
        function submitChatUpdate() {
            ...
            $.ajax({
                url: '{{ route('user.update.ai.docuassistant.details') }}',
                type: 'POST',
                data: formData,
                headers: {
    ...
    <script>
        ...
        function submitChatDelete() {
            ...
            // AJAX call to updateChatDetails
            $.ajax({
                url: '{{ route('user.delete.ai.docuassistant') }}',
                type: 'POST',
                data: formData,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // For CSRF protection in Laravel
    ...
    <script>
        function validateFile() {
            ...
            const allowedExtensions = /(\.docx?|\.pdf|\.xlsx?|\.txt|\.csv)$/i;
            const errorMessage = document.getElementById('error-message');

            if (!allowedExtensions.exec(filePath)) {
                // Set the message inside the alert
                $('#failed-message').text(`{{ __('Invalid file type. Please upload a .doc, .docx, .pdf, .xlsx, .txt, or .csv file.') }}`);
                ...
                // Set the message inside the alert
                $('#success-message').text(`{{ __('File uploaded successfully and entering your question.') }}`);
    ...
    <div class="row align-items-end mb-2 justify-content-end">
        ...
        <div class="col-auto" style="margin-bottom: 10px">
            <span class="avatar" style="background-image: url({{ asset(Auth::user()->profile_image ?: 'images/profile.png') }})"></span>
        </div>
    </div>`;
    ...
    <div class="row align-items-end mb-2 justify-content-start">
        <div class="col-auto" style="margin-bottom: 10px">
            <span class="avatar" style="background-image: url({{ asset($docuAssistArray['chat_genius_image']) }})"></span>
        </div>
        <div class="col col-lg-6">
            <div class="chat-bubble chat-bubble-them">
                <div class="chat-bubble-title">
                    <div class="row">
                        <div class="col chat-bubble-author">{{ $docuAssistArray['chat_genius_name'] }}</div>
                ...
                // Send AJAX request
                $.ajax({
                    url: "{{ route('user.generate.ai.docuassistant') }}",
                    type: "POST",
                    data: formData,
                    contentType: false, // Important for file uploads
                    processData: false, // Prevent jQuery from processing the data
                    success: function(response) {
                        var message = response.docuAssistArray;
