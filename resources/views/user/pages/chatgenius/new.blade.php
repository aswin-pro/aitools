...
@section('custom-css')
    <style>
        ...
        @media (min-height: 800px) {
            ...
            /* Adjust the min-width value as necessary for your design */
            #scrollable {
                height: 30rem;
...
if (!function_exists('wrapPhpWithPre')) {
    ...
    if (!is_string($code)) {
        return htmlspecialchars($code); // Return safely if not a string
    }
        ...
        <div class="container-fluid">
            ...
            <div class="page-header d-print-none">
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <h2 class="page-title">
                            {{ __('Chat with ' . $chatgeniusArray['chat_genius_name']) }}
                        </h2>
                    </div>
                    {{-- New Chat Genius --}}
                    <div class="col-auto ms-auto">
                        <a href="{{ route('user.new.ai.chatgenius.conversation', $chatgeniusArray['chat_genius_id']) }}"
            ...
            <div class="page-body">
                <div class="container-fluid my-auto">
                    ...
                    {{-- Mobile chats modal --}}
                    @if (count($chatgeniusArray['chats']) > 0)
                                    ...
                                    <div class="modal-body">
                                        <div class="card-body p-0 scrollable">
                                            <div class="nav flex-column nav-pills" id="pills-tab" role="tablist"
                                                ...
                                                {{-- Conversations --}}
                                                @if (count($chatgeniusArray['chats']) > 0)
                                                    @foreach ($chatgeniusArray['chats'] as $chat)
                                                            ...
                                                            <div class="row align-items-center flex-fill">
                                                                <div class="col-auto">
                                                                    <span class="avatar rounded-circle"
                                                                        style="background-image: url({{ asset($chatgeniusArray['chat_genius_image']) }})"></span>
                                                                </div>
                                                                <div class="col text-body">
                                                                    <div>{{ __($chat['chat_title']) }}</div>
                                                                    <div class="text-secondary text-truncate w-100">
                                                                        {{ __($chatgeniusArray['chat_genius_expert']) }}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </a>
                                                ...
                                                <a href="#chat{{ $chat['chat_id'] }}"
                                                    ...
                                                    <div class="row align-items-center flex-fill">
                                                        ...
                                                        <div class="col text-body">
                                                            <div>{{ __($chat['chat_title']) }}</div>
                                                            <div class="text-secondary text-truncate w-100">
                                                                {{ __($chatgeniusArray['chat_genius_expert']) }}</div>
                                                        </div>
                                                    </div>
                                                </a>
                                    ...
                                    @if (count($chatgeniusArray['chats']) > 0)
                                        @foreach ($chatgeniusArray['chats'] as $chat)
                                            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                                ...
                                                <div class="chat-bubbles position-relative">
                                                    {{-- Conversations --}}
                                                                            ...
                                                                            <div class="row">
                                                                                <div class="col chat-bubble-author">
                                                                                    @if ($message['responsed_by'] == 'user')
                                                                                        {{ Auth::user()->name }}
                                                                                    @else
                                                                                        {{ $chatgeniusArray['chat_genius_name'] }}
                                                                                    @endif
                                                                                </div>
                                    ...
                                    <form id="chatgenius-form">
                                        <div class="input-group input-group-flat">
                                            ...
                                            <input type="hidden" name="chat_id"
                                                value="{{ $chatgeniusArray['chats'][0]['chat_id'] }}">
...
@section('custom-js')
    ...
    <script>
        ...
        function submitChatUpdate() {
            ...
            $.ajax({
                url: '{{ route('user.update.ai.chatgenius.details') }}',
                type: 'POST',
                data: formData,
                headers: {
                ...
                error: function(xhr, status, error) {
                    ...
                    // Set the message inside the alert
                    $('#failed-message').text(`{{ __('Chat updated failed') }}`);
    ...
    <script>
        ...
        function submitChatDelete() {
            ...
            // AJAX call to updateChatDetails
            $.ajax({
                url: '{{ route('user.delete.ai.chatgenius') }}',
                type: 'POST',
                data: formData,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // For CSRF protection in Laravel
                ...
                error: function(xhr, status, error) {
                    ...
                    console.error(error);
                    // Set the message inside the alert
                    $('#failed-message').text(`{{ __('Chat deleted failed') }}`);
        ...
        $(document).ready(function() {
            $('#chatgenius-form').on('submit', function(event) {
                ...
                <div class="row align-items-end mb-2 justify-content-end">
                    <div class="col col-lg-6">
                        <div class="chat-bubble chat-bubble-them">
                            ...
                            <div class="chat-bubble-body">
                                <p id="chatMessage-${random_chat_id}">${wrapPhpWithPre(user_message)}</p>
                ...
                <div class="row align-items-end mb-2 justify-content-start">
                    <div class="col-auto" style="margin-bottom: 10px">
                        <span class="avatar rounded-circle" style="background-image: url({{ asset($chatgeniusArray['chat_genius_image']) }})"></span>
                    </div>
                    <div class="col col-lg-6">
                        <div class="chat-bubble chat-bubble-them">
                            <div class="chat-bubble-title">
                                <div class="row">
                                    <div class="col chat-bubble-author">{{ $chatgeniusArray['chat_genius_name'] }}</div>
                                    <div class="col-auto chat-bubble-date">${moment(message.created_at).fromNow()}</div>
                                </div>
                            </div>
                            <div class="chat-bubble-body">
                                <p id="chatMessage-${generate_chat_id}">{{ __('Thinking') }}<span class="animated-dots"></span></p>
                ...
                // Send AJAX request
                $.ajax({
                    url: "{{ route('user.generate.ai.chatgenius') }}",
                    type: "POST",
                    data: formData,
                    success: function(response) {
                        var message = response.chatgeniusArray;
                        var chatBubble =
                            `<p id="chatMessage-${message.chat_message_id}"></p>`;
