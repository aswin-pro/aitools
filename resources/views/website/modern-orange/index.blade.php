...
@section('content')
    ...
    <section class="relative pt-20 pb-24 md:pb-20 lg:pt-32 overflow-hidden">
        <img class="absolute bottom-0 left-0"
            src="{{ asset('themes/modern-orange/assets/images/features/start-left-bottom.png') }}" alt="">
        <img class="absolute bottom-1/2 right-0 transform translate-y-1/2"
            src="{{ asset('themes/modern-orange/assets/images/features/blue-light-right.png') }}" alt="">
        <div class="relative container px-4 mx-auto">
            <div class="max-w-7xl mx-auto">
                <div class="flex flex-wrap items-center -mx-4 mb-24">
                    <div class="w-full lg:w-2/3 px-4 mb-16 lg:mb-0">
                        <div class="max-w-lg lg:max-w-2xl mx-auto lg:mx-0">
                            ...
                            <p class="max-w-xl text-2xl md:text-3xl font-semibold text-gray-400">
                    ...
                    <div class="w-full lg:w-1/3 px-4">
                        <img class="w-full max-w-lg mx-auto lg:mr-0"
                            src="{{ asset('themes/modern-orange/assets/images/features/violet-image.png') }}"
                            alt="">
                    </div>
                </div>
                <div class="flex flex-wrap -mx-4">
                    ...
                    @foreach ($tools as $tool)
                        <div
                            class="w-full lg:w-1/3 my-12 lg:mb-0 transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-110">
                            <a href="{{ route('dashboard.user.content-generator.generate', $tool->unique_slug) }}">
                                <div
                                    class="max-w-xs mx-auto pb-10 lg:pb-5 text-center border-b border-gray-100 lg:border-b-0 shadow-xl bg-gray-100 rounded-3xl p-5">
                                    <div
                                        class="flex mx-auto mb-5 items-center justify-center w-15 h-15 rounded-full bg-orange-400">
                                        <img src="{{ asset('themes/modern-orange/assets/images/features/icon-point.svg') }}"
                                            alt="">
                                    </div>
            ...
            <div class="w-full px-4 my-12 lg:mb-0 lg:border-r border-gray-100 text-center">
                <a class="relative group inline-block py-4 px-6 text-white font-semibold bg-orange-900 rounded-full overflow-hidden"
                    ...
                    @if (count($images) > 0)
                        @foreach ($images as $image)
                            <div class="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 px-4 mb-8">
                                <a class="spotlight" href="{{ asset($image->result[0]) }}">
                                    <div
                                        class="max-w-xs md:max-w-none mx-auto h-full pt-3 px-3 pb-5 bg-white transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-110">
                                        <img class="block w-full mb-4 h-64 object-cover"
                                            src="{{ asset($image->result[0]) }}" alt="">
                                        <div class="text-center">
                                            <h5 class="text-xl font-bold capitalize">{{ __($image->name) }}</h5>
                                            <span
                                                class="text-sm font-bold text-gray-400 capitalize">{{ __($image->type) }}</span>
                                        </div>
                                    </div>
                                </a>
                            </div>
                            ...
                            <div class="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 px-4 mb-8">
                                <a class="spotlight" href="{{ asset('images/generate-images/ai/0000' . $i . '.png') }}">
                                    <div
                                        class="max-w-xs md:max-w-none mx-auto h-full pt-3 px-3 pb-5 bg-white transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-110">
                                        <img class="block w-full mb-4 h-64 object-cover"
                                            src="{{ asset('images/generate-images/ai/0000' . $i . '.png') }}"
                                            alt="">
                                        <div class="text-center">
                                            <h5 class="text-xl font-bold capitalize">{{ __('Generated by OpenAI') }}</h5>
                                            <span
                                                class="text-sm font-bold text-gray-400 capitalize">{{ __('Concept Art') }}</span>
                                        </div>
                                    </div>
                                </a>
                            </div>
                        ...
                        <div class="w-full lg:w-1/3 px-4 my-8 lg:mb-0">
                            <div
                                ...
                                <div class="text-center mb-6">
                                    ...
                                    @if ($plan->price == 0)
                                        <span
                                            class="block text-5xl font-bold text-gray-900 mb-3">{{ __('Free') }}</span>
                                    @endif

                                    <span
                                        class="block text-5xl font-bold text-gray-900 mb-3">{{ $plan->price == 0 ? '' : formatCurrency($plan->price) }}</span>
                                ...
                                <ul>
                                    <li class="flex mb-4 items-center">
                                        <img src="{{ asset('themes/modern-orange/assets/images/pricing/green-check.svg') }}"
                                            alt="" /><span
                                            class="ml-2 text-gray-900 font-medium">{{ $plan->template_counts }}
                                            {{ __('Templates') }}</span>
                                    </li>
                                    <li class="flex mb-4 items-center">
                                        <img src="{{ asset('themes/modern-orange/assets/images/pricing/green-check.svg') }}"
                                            alt="" /><span
                                            class="ml-2 text-gray-900 font-medium">{{ $plan->max_words == 9999 ? __('Unlimited') : $plan->max_words }}
                                            {{ __('AI Words') }}</span>
                                    </li>
                                    <li class="flex mb-4 items-center">
                                        <img src="{{ asset('themes/modern-orange/assets/images/pricing/green-check.svg') }}"
                                            alt="" /><span
                                            class="ml-2 text-gray-900 font-medium">{{ $plan->max_images == 9999 ? __('Unlimited') : $plan->max_images }}
                                            {{ __('AI Images') }}</span>
                                    </li>
                                    <li class="flex mb-4 items-center">
                                        <img src="{{ asset($plan->ai_speech_to_text == 1 ? 'themes/modern-orange/assets/images/pricing/green-check.svg' : 'themes/modern-orange/assets/images/pricing/check-circle-grey.svg') }}"
                                            alt="" /><span
                                            class="ml-2 font-medium {{ $plan->ai_speech_to_text == 1 ? 'text-gray-900' : 'text-gray-500 line-through' }}">{{ __('AI Speech to Text') }}</span>
                                    </li>
                                    <li class="flex mb-4 items-center">
                                        <img src="{{ asset($plan->ai_text_to_speech == 1 ? 'themes/modern-orange/assets/images/pricing/green-check.svg' : 'themes/modern-orange/assets/images/pricing/check-circle-grey.svg') }}"
                                            alt="" /><span
                                            class="ml-2 font-medium {{ $plan->ai_text_to_speech == 1 ? 'text-gray-900' : 'text-gray-500 line-through' }}">{{ __('AI Text to Speech') }}</span>
                                    </li>
                                    <li class="flex mb-4 items-center">
