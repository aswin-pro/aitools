...
namespace Plugins\MSG91SmsNotification\Controllers;
...
{
    public function msg91SmsNotificationSettings(Request $request)
    {
        // check database and create table if not exists
        if (!Schema::hasTable('msg91_sms_notification_settings')) {
            // Create table
            Schema::create('msg91_sms_notification_settings', function (Blueprint $table) {
                $table->id();
        ...
        // Check if table msg91_sms_notification_templates exists
        if (!Schema::hasTable('msg91_sms_notification_templates')) {
            // Create table
            Schema::create('msg91_sms_notification_templates', function (Blueprint $table) {
                $table->id();
            ...
            DB::table('msg91_sms_notification_templates')->insert([
                [
                    'id'            => 1,
        ...
        $msg91_sms_notification_settings  = DB::table('msg91_sms_notification_settings')->first();

        // notification templates
        $msg91_sms_notification_templates =
            DB::table('msg91_sms_notification_templates')
            ->get()
            ->keyBy('template_name');
    ...
    public function msg91SmsNotificationSettingsUpdate(Request $request)
    {
        // Validate request
        ...
        // Update or insert
        DB::table('msg91_sms_notification_settings')->updateOrInsert(
            ['id' => 1],
            [
        ...
        // forget cache
        cache()->forget('msg91_sms_notification_settings');

        // redirect to settings
        return redirect()->back()->with('success', __('MSG91 Sms Notification Settings Updated Successfully!'));
    }

    // User Registration Template Update
    public function msg91SmsTemplateUserRegisterUpdate(Request $request)
    {
        // validate request
    ...
    public function msg91SmsTemplatePlanPurchaseUpdate(Request $request)
    {
        // validate request
        if ($r = $this->validateTemplate($request, [
    ...
    public function msg91SmsTemplatePlanRenewalUpdate(Request $request)
    {
        // validate request
        if ($r = $this->validateTemplate($request, [
    ...
    public function msg91SmsTemplateUserPlanExpiryRemainderUpdate(Request $request)
    {
        // validate request
        if ($r = $this->validateTemplate($request, [
    ...
    public function msg91SmsTemplateUserExpiredUpdate(Request $request)
    {
        // validate request
        if ($r = $this->validateTemplate($request, [
    ...
    private function validateTemplate(Request $request, array $rules)
    {
        ...
        if ($validator->fails()) {
            return redirect()
                ->route('admin.plugin.msg91_sms_notification.settings')
